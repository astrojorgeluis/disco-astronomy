...
const AnalysisDashboard = ({
    ...
    useEffect(() => {
        ...
        setRenderedImages(prev => {
            const next = { ...prev };
            ['data', 'deproj', 'model', 'residuals', 'polar'].forEach(k => {
                if (!next[k] && results[k]) next[k] = results[k];
            });
            return next;
        });
    }, [results]);
    ...
    useEffect(() => {
        setVizStats(prev => {
            if (!prev) return prev;
            const raw = (prev.cmap_used || vizParams.cmap || 'magma').replace('_r', '');
            const next = invertCmap ? `${raw}_r` : raw;
            if (next === prev.cmap_used) return prev;
            return { ...prev, cmap_used: next };
        });
    }, [invertCmap, vizParams.cmap]);

    useEffect(() => { updateVisualization({}); }, [viewType]);
    ...
    const handleSaveMarker = () => {
        if (tempMarkerPos) {
            setMarkers(prev => [...prev, { x: tempMarkerPos.x, y: tempMarkerPos.y, label: newMarkerName, shape: newMarkerShape, color: newMarkerColor, view: viewType }]);
        }
    ...
    const handleMouseMove = (e) => {
        if (activeTool === 'inspector' && e && e.activePayload) setSyncedRadius(e.activePayload[0].payload.radius);
        else if (activeTool === 'inspector') setSyncedRadius(null);
        if (isDragging && e && e.activeLabel) setDragEnd(Number(e.activeLabel));
    };
    ...
    const updateVisualization = async (overrideParams = {}) => {
        ...
        if (overrideParams.hasOwnProperty('vmax_percentile')) {
            payload.vmax_percentile = overrideParams.vmax_percentile;
            payload.vmin = null;
            payload.vmax = null;
        } else {
            let valMin = parseFloat(manualMinStr);
            let valMax = parseFloat(manualMaxStr);
            if (isNaN(valMin)) valMin = vizStats?.vmin_used ?? 0.0;
            if (isNaN(valMax)) valMax = vizStats?.vmax_used ?? 4.0;
            payload.vmin = valMin;
            payload.vmax = valMax;
        ...
        setVizParams(prev => ({
            ...
            cmap: baseCmap,
            stretch: finalStretch,
            vmax_percentile: payload.vmax_percentile ?? prev.vmax_percentile,
        ...
        try {
            ...
            if (data.image) setRenderedImages(prev => ({ ...prev, [viewType]: data.image }));
            if (data.stats) {
                const stats = { ...data.stats, cmap_used: finalCmap };
                setVizStats(stats);
                setManualMinStr(String(stats.vmin_used));
                setManualMaxStr(String(stats.vmax_used));
            }
        } catch {}
    };
    ...
    useEffect(() => {
        if (vizStats) {
            setManualMinStr(String(vizStats.vmin_used));
            setManualMaxStr(String(vizStats.vmax_used));
        }
    }, [vizStats]);
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--disco-bg-panel)', overflow: 'hidden' }}>
            ...
            <Split sizes={[60, 40]} minSize={300} gutterSize={4} className="split-flex">
                <div style={{ position: 'relative', overflow: 'hidden', background: 'var(--disco-chart-bg)', width: '100%', height: '100%' }}>
                    <SimpleImageViewer
                        ref={viewerRef}
                        imageSrc={renderedImages[viewType] || currentImages[viewType]}
                        type={viewType}
                    ...
                    <div className="chart-container" style={{ flex: 1, padding: 10, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                            ...
                            <div style={{ display: 'flex', gap: 5 }}>
                                <Button icon="download" text="CSV" intent="success" minimal={true} small={true} title="Download data as CSV" disabled={!profileData} onClick={handleDownloadCSV} />
                        ...
                        <div style={{ flex: 1, minHeight: 0 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    ...
                                    {safeRange && <ReferenceArea x1={safeRange.x1} x2={safeRange.x2} strokeOpacity={0.3} fill="var(--disco-selection)" fillOpacity={0.1} />}
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
