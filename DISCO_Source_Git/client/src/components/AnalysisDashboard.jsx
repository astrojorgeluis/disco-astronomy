...
import { 
    ...
    Dialog, Classes, InputGroup, Label, NonIdealState
...
const AnalysisDashboard = ({ 
    ...
    const [vizStatsByView, setVizStatsByView] = useState({});
    ...
    const [vizParams, setVizParams] = useState({ ...DEFAULT_VIZ });
    ...
    const vizStats = vizStatsByView[viewType] || null;

    const handlePlotClick = (pos) => {
        if (isMarkerMode) {
            setTempMarkerPos(pos);
            setNewMarkerName(`Point ${markers.length + 1}`);
    ...
    const handleSaveMarker = () => {
        if (tempMarkerPos) {
            setMarkers(prev => [...prev, {
                x: tempMarkerPos.x, y: tempMarkerPos.y,
                label: newMarkerName, shape: newMarkerShape,
                color: newMarkerColor, view: viewType
            }]);
        }
    ...
    const handleDownloadCSV = () => {
        ...
        radius.forEach((r, i) => {
            const rawVal = raw_intensity ? raw_intensity[i] : 0;
            const tbVal = intensity[i];
            rows.push([r, rawVal, tbVal]);
        });
    ...
    const updateVisualization = useCallback(async (overrideParams = {}) => {
        if (!results) return;

        const nextParams = { ...vizParams, ...overrideParams };
        let baseCmap = (overrideParams.cmap ?? vizParams.cmap).replace('_r', '');
        const finalCmap = invertCmap ? `${baseCmap}_r` : baseCmap;
        const finalStretch = overrideParams.stretch ?? vizParams.stretch;
        const useAuto = overrideParams.hasOwnProperty('vmax_percentile')
            ? overrideParams.vmax_percentile != null
            : (autoScaleRef.current || nextParams.vmax_percentile != null);
        ...
        if (useAuto) {
            payload.vmax_percentile = overrideParams.vmax_percentile ?? nextParams.vmax_percentile ?? 100;
            payload.vmin = null;
            payload.vmax = null;
            autoScaleRef.current = true;
        } else {
            let valMin = parseFloat(manualMinStr);
            let valMax = parseFloat(manualMaxStr);
            if (isNaN(valMin)) valMin = 0.0;
            if (isNaN(valMax)) valMax = 1.0;
            payload.vmin = valMin;
            payload.vmax = valMax;
            payload.vmax_percentile = null;
        ...
        try {
            ...
            if (data.image) {
                setCurrentImages(prev => ({ ...prev, [viewType]: data.image }));
            }
            if (data.stats) {
                const stats = { ...data.stats, cmap_used: finalCmap };
                setVizStatsByView(prev => ({ ...prev, [viewType]: stats }));
                if (autoScaleRef.current) {
                    setManualMinStr(String(data.stats.vmin_used));
                    setManualMaxStr(String(data.stats.vmax_used));
                }
            }
        } catch (e) {
    ...
    }, [results, viewType, invertCmap]);

    const chartData = useMemo(() => {
        if (!profileData) return [];
        return profileData.radius.map((r, i) => {
            let val = profileData.intensity[i];
            if (useLogScale && val <= 0) val = 0.001;
            return { radius: r, intensity: val };
        });
    ...
    const cumulativeData = useMemo(() => {
        ...
        let cumSum = 0;
        const data = [];
        const radii = profileData.radius;
        const intensities = profileData.intensity;
        for (let i = 0; i < radii.length; i++) {
            const r = radii[i];
            const val = Math.max(0, intensities[i] || 0);
            cumSum += val * r;
            data.push({ radius: r, cumulative: cumSum });
        }
        const total = cumSum > 0 ? cumSum : 1;
        return data.map(d => ({ radius: d.radius, cumulative: (d.cumulative / total) * 100 }));
    }, [profileData]);
    ...
    const vizSettingsContent = (
        <div style={{padding: 15, width: 300, background:'var(--disco-bg-panel)', border:'1px solid var(--disco-border)'}}>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:10}}>
                <NumericInput fill value={manualMinStr} onValueChange={(_, vStr) => { autoScaleRef.current = false; setManualMinStr(vStr); }} buttonPosition="none" placeholder="Min"/>
                <NumericInput fill value={manualMaxStr} onValueChange={(_, vStr) => { autoScaleRef.current = false; setManualMaxStr(vStr); }} buttonPosition="none" placeholder="Max"/>
            </div>
            <div style={{display:'flex', gap:5, marginBottom:15}}>
                <Button fill small text="Apply" icon="refresh" onClick={() => { autoScaleRef.current = false; updateVisualization({ vmax_percentile: null }); }}/>
                <Button fill small text="Auto" onClick={() => { autoScaleRef.current = true; updateVisualization({ vmax_percentile: 100 }); }}/>
            </div>
    ...
    if (!results) {
        return (
            <div style={{height:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:'var(--disco-bg-panel)'}}>
                <NonIdealState
                    icon="play"
                    title="No analysis yet"
                    description="Load a FITS file, set the disk geometry, then click RUN PIPELINE to see deprojected views, radial profiles, and fit tools."
                />
            </div>
        );
    }
            ...
            <Split sizes={[60, 40]} minSize={300} gutterSize={4} className="split-flex">
                <div style={{position:'relative', overflow:'hidden', background:'var(--disco-chart-bg)', width:'100%', height:'100%'}}>
                    <SimpleImageViewer 
                        ...
                        pixelScale={geometry ? geometry.pixel_scale : 0.03}
