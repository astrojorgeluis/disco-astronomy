...
const ColorBar = ({ cmap, vizStats }) => {
    const gradients = {
        ...
        plasma: 'linear-gradient(to top, #0d0887, #7e03a8, #cc4778, #f89441, #f0f921)',
        viridis: 'linear-gradient(to top, #440154, #31688e, #35b779, #fde725)',
        cividis: 'linear-gradient(to top, #00204c, #424086, #7e7c78, #bcae6c, #fee838)',
    ...
    const isReversed = rawCmap.endsWith('_r');
    const base = rawCmap.replace('_r', '');
    const baseGradient = gradients[base] || gradients.magma;
    const bg = isReversed ? baseGradient.replace('to top', 'to bottom') : baseGradient;

    const min = Number.isFinite(vizStats?.vmin_used) ? vizStats.vmin_used : 0;
    const max = Number.isFinite(vizStats?.vmax_used) ? vizStats.vmax_used : 1;
    const mid = (min + max) / 2;
    const textColor = 'var(--disco-text-muted)';
    const borderColor = 'var(--disco-border)';

    return (
        <div key={`${rawCmap}-${min}-${max}`} style={{ position: 'absolute', right: 10, bottom: 60, top: 60, width: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', pointerEvents: 'none', zIndex: 5 }}>
            ...
            <div style={{ flex: 1, width: 14, background: bg, border: `1px solid ${borderColor}`, position: 'relative' }}>
                <div style={{ position: 'absolute', top: '50%', right: 0, width: 4, height: 1, background: textColor }} />
...
const SimpleImageViewer = forwardRef(({
    ...
    const handleStageMouseUp = (e) => {
        ...
        if (isMarkerMode && onPlotClick && image) {
            const stage = e.target.getStage();
            const ptr = stage.getPointerPosition();
            const imgX = (ptr.x - transform.x) / transform.k;
            const imgY = (ptr.y - transform.y) / transform.k;
    ...
    const renderRangeOverlay = () => {
        ...
        let rMin, rMax;
        if (activeTool === 'inspector' && dragStartR !== null) {
            rMin = Math.min(dragStartR, dragCurrentR);
            rMax = Math.max(dragStartR, dragCurrentR);
        } else if (currentFitRange && currentFitRange.length === 2 && currentFitRange[1] > currentFitRange[0]) {
            rMin = currentFitRange[0];
            rMax = currentFitRange[1];
        } else return null;
    ...
    const renderSync = () => {
        ...
        if (type === 'polar') {
            const x = externalHoverRadius * pxPerArcsec;
            return <Line points={[x, 0, x, image.height]} stroke={SYNC_COLOR} strokeWidth={2 / transform.k} dash={[10, 5]} listening={false} />;
        }
    ...
    const renderMarkers = () => {
        ...
        return markers.map((m, i) => {
            ...
            const strokeW = 2 / transform.k;
            let ShapeComp = Circle;
            let shapeProps = { radius: size, fill: 'transparent', stroke: m.color, strokeWidth: strokeW };
            if (m.shape === 'square') {
                ShapeComp = Rect;
                shapeProps = { width: size * 2, height: size * 2, offsetX: size, offsetY: size, stroke: m.color, strokeWidth: strokeW };
            } else if (m.shape === 'star') {
                ShapeComp = Star;
                shapeProps = { numPoints: 5, innerRadius: size / 2, outerRadius: size, stroke: m.color, strokeWidth: strokeW };
            } else if (m.shape === 'cross') {
                return (
                    <Group key={i} x={m.x} y={m.y}>
                        <Line points={[-size, -size, size, size]} stroke={m.color} strokeWidth={strokeW} />
                        <Line points={[size, -size, -size, size]} stroke={m.color} strokeWidth={strokeW} />
                        <Text text={m.label} x={size + 5 / transform.k} y={-size} fill={m.color} fontSize={12 / transform.k} fontStyle="bold" shadowColor="black" shadowBlur={2} />
                    </Group>
                );
            }
            return (
                <Group key={i} x={m.x} y={m.y}>
                    <ShapeComp {...shapeProps} />
                    <Text text={m.label} x={size + 5 / transform.k} y={-size} fill={m.color} fontSize={12 / transform.k} fontStyle="bold" shadowColor="black" shadowBlur={2} />
                </Group>
            );
        });
    ...
    return (
        <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', background: 'var(--disco-chart-bg)', overflow: 'hidden', cursor: activeTool === 'inspector' ? 'crosshair' : (isMarkerMode ? 'crosshair' : (activeTool === 'pan' ? 'grab' : 'default')) }}>
            <Stage
                ...
                onMouseLeave={() => {
                    if (onHoverRadius) onHoverRadius(null);
                    setCursorVal(null);
                ...
                draggable={activeTool === 'pan' && dragStartR === null}
                x={transform.x}
                y={transform.y}
                scaleX={transform.k}
                scaleY={transform.k}
                ...
                onDragEnd={(e) => {
                    if (activeTool !== 'pan') {
                        e.target.position({ x: transform.x, y: transform.y });
                    }
                }}
                style={{ opacity: dims.w > 0 ? 1 : 0, transition: 'opacity 0.2s' }}
            >
                <Layer>
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
                {dims.w > 0 && showAxes && (
                    <Stage width={dims.w} height={dims.h}>
                        <Layer>
                            {image && <DynamicAxes width={dims.w} height={dims.h} fieldOfView={fieldOfView} transform={transform} type={type} imgSize={{ w: image.width, h: image.height }} />}
                            {renderBeamHUD()}
                        </Layer>
                    </Stage>
                )}
