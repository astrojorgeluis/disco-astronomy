...
const SmartInput = ({ value, onValueChange, ...props }) => {
    ...
    useEffect(() => {
        ...
        if (value !== undefined && (Math.abs(num - value) > 1e-6 || isNaN(num))) setStr(String(value));
    }, [value]);
    ...
    const fetchPlot = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setOptions(prev => {
                    if (prev.vmin === null || prev.vmax === null) {
                        return {
                            ...prev,
                            vmin: prev.vmin === null ? data.stats.vmin_used : prev.vmin,
                            vmax: prev.vmax === null ? data.stats.vmax_used : prev.vmax
                        };
                    }
                    return prev;
                });
            ...
            <div className="custom-scroll" style={{ width: 300, padding: 15, borderRight: '1px solid var(--disco-border)', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 15 }}>
                <div style={{ background: 'var(--disco-bg-app)', padding: 10, borderRadius: 4, border: '1px solid var(--disco-border)' }}>
                    ...
                    <HTMLSelect
                        ...
                        large
                        value={options.type}
                        onChange={e => setOptions(prev => ({ ...prev, type: e.target.value, vmin: null, vmax: null }))}
                        options={[
                ...
                {!isProfile && (
                    <>
                        <div>
                            ...
                            <HTMLSelect fill value={options.cmap} onChange={e => updateOption('cmap', e.target.value)} options={['magma', 'inferno', 'viridis', 'gray', 'seismic', 'jet', 'twilight']} />
                        </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 5 }}>
                    ...
                    <Button
                        ...
                        onClick={() => {
                            if (!image) return;
                            const filename = `plot_${options.type}.png`;
                            ...
                            setSaveMsg(`PNG saved to your browser Downloads folder as ${filename}`);
                        }}
                        disabled={!image}
                    />
                </div>

                {saveMsg && <div style={{ fontSize: 11, color: 'var(--disco-success)', marginTop: 6 }}>{saveMsg}</div>}
